...
import com.il360.shenghecar.model.alipay.AliBank;
import com.il360.shenghecar.model.alipay.AliUserInfo;
import com.il360.shenghecar.model.alipay.AlipayInfo;
import com.il360.shenghecar.model.alipay.ArrayOfBankInfo;
import com.il360.shenghecar.model.alipay.BankInfo;
import com.il360.shenghecar.model.alipay.BillItem;
import com.il360.shenghecar.model.alipay.BillList;
import com.il360.shenghecar.model.alipay.RsaUi;
import com.il360.shenghecar.util.FastJsonUtils;
import com.il360.shenghecar.util.FileHelper;
		...
		public void onPageFinished(WebView view, String url) {
			...
			if (url.endsWith("alipay.com/login/checkSecurity.htm?full_redirect=true")) {
				...
				String js2 = "function removeClassName(name){var eles = document.getElementsByClassName(name);var i;for (i = 0; i < eles.length; i++) {eles[i].remove();}};"
						+ "function setupLoginHomeUI(){removeClassName('authcenter-head');removeClassName('qrcode-footer-help');removeClassName('ui-form-other ui-form-other-fg');removeClassName('ui-form-other');removeClassName('authcenter-foot');};"
						+ "setupLoginHomeUI();";
			...
			try {
				...
				if (result.getSuccess()) {
					if (ResultUtil.isOutTime(result.getResult()) != null) {
						...
						Intent intent = new Intent(HuaBeiActivity.this, LoginActivity_.class);
...
//				aliUserInfo.setAmount(new BigDecimal(total.trim()));
				...
				BigDecimal availableB = new BigDecimal(available.trim());
				BigDecimal totalB = new BigDecimal(total.trim());
				
				BigDecimal usedAmount = totalB.subtract(availableB);
				aliUserInfo.setUsedAmount(usedAmount);
				aliUserInfo.setAmount(totalB);
