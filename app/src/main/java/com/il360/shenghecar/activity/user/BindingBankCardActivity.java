	...
	private void initVerified() {
		...
		if (jsonStr != null) {
			try {
				...
				TResult<Boolean, String> result = HttpRequestUtil.sendPostRequest(UrlEnum.BIZ_URL, "card/bindingBank",params);
				if (result.getSuccess()) {
					if (ResultUtil.isOutTime(result.getResult()) != null) {
						showInfo(ResultUtil.isOutTime(result.getResult()));
						Intent intent = new Intent(BindingBankCardActivity.this, LoginActivity_.class);
