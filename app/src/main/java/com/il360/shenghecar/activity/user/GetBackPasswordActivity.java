	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			if (msg.what > 0) {
				...
				if (!ViewUtil.getText(etAccount).equals("")) {//空
		...
		if (ok()) {
			ExecuteTask.execute(new Runnable() {
				...
				public void run() {
					try {
						Map<String, String> params = new HashMap<String, String>();
						params.put("phone", etAccount.getText().toString());//et类型赋值
						params.put("verifyType", "2");// 2找回登入密码
						...
						TResult<Boolean, String> result = HttpRequestUtil.sendPostRequest(UrlEnum.BIZ_URL, "user/validUserVerifyCode", params);
						if (result.getSuccess()) {
							if (ResultUtil.isOutTime(result.getResult()) != null) {
								showInfo(ResultUtil.isOutTime(result.getResult()));
								Intent intent = new Intent(GetBackPasswordActivity.this, LoginActivity_.class);
