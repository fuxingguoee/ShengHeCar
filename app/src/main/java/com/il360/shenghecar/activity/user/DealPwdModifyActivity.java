			...
			ExecuteTask.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						TResult<Boolean, String> result = HttpRequestUtil.sendPostRequest(UrlEnum.BIZ_URL, "user/savePayPwd", params);
						if (result.getSuccess()) {
							if (ResultUtil.isOutTime(result.getResult()) != null) {
								showInfo(ResultUtil.isOutTime(result.getResult()));
								Intent intent = new Intent(DealPwdModifyActivity.this, LoginActivity_.class);
								...
								if (obj.getInt("code") == 1) {
									...
									if (objRes.getInt("returnCode") == 1) {
										GlobalPara.getOutUserRz().setPayPwd(1);
